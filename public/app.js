// public/app.js
// Full frontend script (search wired, image fallbacks, suggested/popular integrated)
...
const toastEl = $('toast');
const toast = (m, ms=1600) => { if(!toastEl) return; toastEl.textContent=m; toastEl.style.display='block'; setTimeout(()=>toastEl.style.display='none', ms); };

// Universal fallback image (used when provider has no photo)
...
// Load external provider destinations (proxied)
...
// runSearch: builds params and calls loadTrips
const runSearch = () => {
  const params = {};
...
// view toggles
$('viewGrid')?.addEventListener('click', ()=> { TRIP_VIEW='grid'; runSearch(); $('viewGrid').style.background='#00b3b3'; $('viewList').style.background='#555'; });
$('viewList')?.addEventListener('click', ()=> { TRIP_VIEW='list'; runSearch(); $('viewList').style.background='#00b3b3'; $('viewGrid').style.background='#555'; });
...
const searchFlights = async () => {
  ...
  try { ff.btn.disabled=true; ff.btn.textContent='Searching...'; const flights = await post(API.flights, { source, destination, departureDate }); renderFlights(flights); toast('Flights loaded'); }
