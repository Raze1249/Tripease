...
const toastEl = $('toast');
const toast = (m, ms=1800) => { if(!toastEl) return; toastEl.textContent = m; toastEl.style.display = 'block'; setTimeout(()=>toastEl.style.display='none', ms); };
...
// ===================== POPULAR PLACES (static gallery + external provider) =====================
...
// Fetch external destinations (proxied by server) and normalize to our card format
async function loadExternalDestinations({ limit = 6, region = '' } = {}) {
  try {
    ...
    return list.map(d => ({
      ...
      blurb: (d.description && (d.description.length>120 ? d.description.slice(0,117)+'...' : d.description)) || d.region || '',
      rating: d.rating || 5,
      id: d.id || d.name
    }));
  } catch (err) {
...
// Render popular: prefer external provider, fallback to static
...
// ===================== SUGGESTED TRIPS (top-rated from API with provider fallback) =====================
...
// Try DB trips, then provider, then fallback
